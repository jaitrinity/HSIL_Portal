...
export class QuotationManagementComponent implements OnInit {
        ...
        if(this.currentQuotationType == "Budgetory"){
          ...
          for(let i=0;i<wrappedList.length;i++){
            ...
            // if(!(projectStage == 'Lost')){
              ...
              if(!this.for_TM && this.loggedInUserRole == "Institutional_Sales" &&
                (
              ...
              if(!this.for_Commercial && this.loggedInUserRole == "HINDWARE_COMM" && (wrappedObj.project_po_id != null)){
  ...
  validateDataForDiscountedQuotation() : any{
    ...
    //   return false;
    // }
    // else if(this.Liability_SW == ''){
    //   $("#Liability_SWInput").focus();
    //   this.toastr.warning("please enter value of `In built TOD/Extra Liability SW (%)` ");
    //   return false;
    // }
    ...
    //   return false;
    // }
    // else if(this.PAY_TERMS_SW == ''){
    //   $("#PAY_TERMS_SWInput").focus();
    //   this.toastr.warning("please select value of `Payment Terms for SW` ");
    //   return false;
    // }
    ...
    //   return false;
    // }
    // else if(this.FREIGHT_SW == ''){
    //   $("#FREIGHT_SWInput").focus();
    //   this.toastr.warning("please enter value of `Freight for SW` ");
    //   return false;
    // }
  ...
  saveSkuData(){
    ...
    let dc = "";
    if(this.project_type == 'Project Channel 16') dc = '16';
    else if(this.project_type == 'Project Channel 23') dc = '23';
    else if(this.project_type == 'Project Channel 11') dc = '11';
    let jsonData = [];
    if(this.currentQuotationType == "Budgetory"){
      let json = {
  ...
  applyDiscount(event : any){
    ...
    if(this.currentQuotationType == "Budgetory"){
      ...
      for(let i=0;i<this.quotationDiscountMatrix.length;i++){
        ...
        let cat = disObj.Catogory == category;
        if(cat){
          isCatFound = true;
          if(dd){
            isFind = true;
  ...
  viewQuotPdf(quotId : any,quotType :any){
    ...
    let pdfUrl = "http://www.fast.org.in:8080/HSILQuotation_test/Service/generatePdfForQuotation?EMP_ID="+this.loggedInEmployeeId+"&QUOT_ID="+quotId+"&line="+line+"&type="+type;
    window.open(pdfUrl)
